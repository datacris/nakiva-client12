...
const Pedido = ({ pedido }) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          console.log("Error confirmarEliminarPedido => ", error);
        }
      }
    });
  };
  ...
  return (
    <div
      className={`${clase} border-t-4 mt-4 bg-white p-6 md:grid md:grid-cols-2 md:gap-4 shadow-lg`}
    >
      <div>
      ...
      <div>
        <h2 className="text-gray-800 font-bold mt-2">Resume del pedido</h2>
        ...
        <button
          className="flex item-center mt-4 bg-red-800 hover:bg-red-900 px-5 py-2 inline-block text-white rounded leading-tight uppercase text-xs font-bold"
